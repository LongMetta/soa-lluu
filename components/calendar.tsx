    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-4">
        {days.map(({ date, isCurrentMonth }, index) => {
          ...
          const isOccupied =
            occupiedDays.includes(dayOfMonth) && isCurrentMonth;
          const isSelected =
          ...
          return (
            <button
              key={index}
              onClick={() =>
                isCurrentMonth && !isOccupied && onSelectDate(date)
              }
              disabled={!isCurrentMonth || isOccupied}
              className={`
                p-2 rounded-xl text-center md:min-h-[80px] flex flex-col items-center justify-between
                ${!isCurrentMonth ? "text-gray-300 bg-gray-50" : ""}
                ${
                  isSelected
                    ? "border-2 border-[#E85C3F]"
            ...
            >
              ...
              {isCurrentMonth && (
                <span
                  className={`hidden md:block md:text-sm text-xs ${
                    isOccupied ? "text-gray-400" : "text-[#E85C3F]"
                  }`}
                >
